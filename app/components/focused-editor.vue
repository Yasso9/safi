<template>
    <div class="relative w-full">
        <div
            ...
            class="relative z-10 min-h-[70vh] w-full border-0 bg-transparent p-8 font-mono text-lg leading-relaxed text-[#0a0a0a] outline-none focus:outline-none dark:text-[#fafafa]"
            spellcheck="false"
        ></div>
        <div
            v-if="isEmpty"
            class="pointer-events-none absolute top-8 left-8 z-20 font-mono text-lg text-[#a3a3a3] dark:text-[#525252]"
        >
            {{ placeholder }}
        </div>
    </div>
...
const updateFocusedParagraph = () => {
    ...
    const paragraphs = editorRef.value.querySelectorAll(
...
const handleKeydown = async (e: KeyboardEvent) => {
    ...
    try {
        ...
        const { textBefore, textAfter } = splitParagraphAtCursor(
            currentParagraph,
            range,
        )
        ...
        console.log(
            '📊 [DOM] Current paragraph now has:',
            `"${(currentParagraph.textContent || '').replace(/\u200B/g, '[ZERO-WIDTH]')}"`,
        )
        console.log(
            '📊 [DOM] New paragraph has:',
            `"${(newParagraph.textContent || '').replace(/\u200B/g, '[ZERO-WIDTH]')}"`,
        )
        console.log(
            '📊 [DOM] Both have same content?',
            currentParagraph.textContent === newParagraph.textContent,
        )
...
const splitParagraphAtCursor = (
    ...
    if (range.startContainer === paragraph) {
        ...
        if (range.startOffset === 0) {
            console.log(
                '✂️ [Split] → Offset 0: before="" after="' + displayText + '"',
            )
            return { textBefore: '', textAfter: fullText }
        } else {
            console.log(
                '✂️ [Split] → Offset > 0: before="'
                    + displayText
                    + '" after=""',
            )
    ...
    while (currentNode) {
        if (currentNode === range.startContainer) {
            ...
            console.log(
                '✂️ [Split] → Found in text node, offset:',
                cursorOffset,
            )
            break
        }
...
const setCursorAtParagraphStart = (paragraph: HTMLElement) => {
    const displayText = (paragraph.textContent || '').replace(
        /\u200B/g,
        '[ZERO-WIDTH]',
    )
    ...
    try {
        ...
        console.log(
            '🎯 [Cursor] First child type:',
            textNode?.nodeType === Node.TEXT_NODE ? 'TEXT_NODE' : 'OTHER',
        )
        ...
        setTimeout(() => {
            ...
            if (verifySelection && verifySelection.rangeCount > 0) {
                ...
                const actualText = (actualParagraph?.textContent || '').replace(
                    /\u200B/g,
                    '[ZERO-WIDTH]',
                )
                console.log(
                    '🔍 [Verify] Cursor is actually in:',
                    `"${actualText}"`,
                )
                console.log(
                    '🔍 [Verify] Is in target paragraph?',
